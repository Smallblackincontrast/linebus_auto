...
# @File     :test_ticket_id_upd_status_check.py
# @software :PyCharm
import unittest
...
testdata = ReadDatas(DataPaths.DatasPath).read_datas("ticket_id_upd_status_check")


@ddt
class TestTicketIdUpdStatusCheck(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.log = MyLogs()
        ...
        cls.transaction = cls.api.api_request("post", CD.url + r'/transaction/creditcard/payrecordpage',
                                              ...
                                              {"pageNo": 0, "pageSize": 20, "cardNum": "", "paypalAccount": "",
                                               "tenantId": CD.tid, "transactionId": "", "authCode": ""})
        setattr(CD, "order_id", cls.transaction.json()["result"][0]["orderId"])
        setattr(CD, "transaction_id", cls.transaction.json()["result"][0]["transactionId"])
        # 未验的票id
        ticket_data = operation_mysql("select * from bus_linebus_engine.lb_ticket where status = 0 and tid = \'" + CD.tid + "\';")
        setattr(CD, "ticket_id", ticket_data[0]["id"])
        # 查询已检票的票
        checked_ticket_data = operation_mysql(
            "select * from bus_linebus_engine.lb_ticket where status = 1 and tid = \'" + CD.tid + "\';")
        setattr(CD, "checked_ticket_id", checked_ticket_data[-1]["id"])
    ...
    @data(*testdata)
    def test_ticket_id_upd_status_check(self, datas):
