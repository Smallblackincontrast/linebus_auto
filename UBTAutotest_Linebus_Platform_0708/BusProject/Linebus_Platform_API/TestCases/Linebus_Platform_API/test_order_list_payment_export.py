# -*- coding:utf-8 -*-
# @Time     :2019/7/24 11:27
# @Author   :Tester_Wang
# @Email    :wangying@riverroad.com
# @File     :test_order_list_payment_export.py
# @software :PyCharm
import unittest
import csv
...
testdata = ReadDatas(DataPaths.DatasPath).read_datas("order_list_payment_export")


@ddt
class TestOrderListPaymentExport(unittest.TestCase):
    ...
    @data(*testdata)
    def test_order_list_payment_export(self, datas):
        ...
        if datas["id"]==1:
            self.log.info("the actual result :{0}".format(res))
        if datas["id"] == 2:
        ...
        try:
            ...
            self.read_data.write_back(datas['module'], datas['id'] + 1, str(res), Result)
