# -*- coding:utf-8 -*-
# @Time     :2019/7/22 14:18
# @Author   :Tester_Wang
# @Email    :wangying@riverroad.com
# @File     :test_ticketrefund_cancelrefund.py
# @software :PyCharm
import unittest
...
testdata = ReadDatas(DataPaths.DatasPath).read_datas("ticketrefund_cancelrefund")


@ddt
class TestTicketrefundCancelrefund(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.log = MyLogs()
        ...
        cls.re = DataRe()
        cls.api = ApiRequests()
        ticket_refund_data = operation_mysql("SELECT * FROM lb_ticket_refund WHERE deleted=0 and tid = \'" + CD.tid + "\';")
    ...
    @data(*testdata)
    def test_ticketrefund_cancelrefund(self, datas):
