...
# @File     :test_ticketrefund_order_pay.py
# @software :PyCharm
import unittest
...
testdata = ReadDatas(DataPaths.DatasPath).read_datas("ticketrefund_order_pay")


@ddt
class TestTicketrefundOrderPay(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.log = MyLogs()
        ...
        ticket_data = operation_mysql(
            "select * from bus_linebus_engine.lb_ticket where status = 8 and tid = \'" + CD.tid + "\';")
    ...
    @data(*testdata)
    def test_ticketrefund_order_pay(self, datas):
