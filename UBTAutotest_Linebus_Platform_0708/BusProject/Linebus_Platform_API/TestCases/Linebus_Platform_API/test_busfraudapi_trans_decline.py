...
# @File     :test_busfraudapi_trans_decline.py
# @software :PyCharm
import unittest
...
testdata = ReadDatas(DataPaths.DatasPath).read_datas("busfraudapi_trans_decline")


@ddt
class TestBusfraudapiTransDecline(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.log = MyLogs()
        ...
        setattr(CD, "credit_card_payment_id", cls.transaction.json()["result"][-1]["id"])
    ...
    @data(*testdata)
    def test_busfraudapi_trans_decline(self, datas):
