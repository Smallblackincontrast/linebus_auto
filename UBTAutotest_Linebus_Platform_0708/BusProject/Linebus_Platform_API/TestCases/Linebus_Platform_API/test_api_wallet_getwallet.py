# -*- coding:utf-8 -*-
# @Time     :2019/7/18 15:11
# @Author   :Tester_Wang
# @Email    :wangying@riverroad.com
# @File     :test_api_wallet_getwallet.py
# @software :PyCharm
import unittest
...
testdata = ReadDatas(DataPaths.DatasPath).read_datas("api_wallet_getwallet")


@ddt
class TestApiWalletGetallet(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.log = MyLogs()
        ...
        # 钱包未激活的用户
        # cls.nowallet_user_login = cls.api.api_request("post", CD.url + r"/api/auth/login", {"tid": str(CD.tid)},
        #                                               {"account": CD.nowallet_user_account, "password": "123456",
        #                                                "client": "1"})
        # setattr(CD, "nowallet_sessionid", cls.nowallet_user_login.json()["result"]["sessionId"])
        # setattr(CD, "nowallet_accesstoken", cls.nowallet_user_login.json()["result"]["authCode"])
    ...
    @data(*testdata)
    def test_api_wallet_getwallet(self, datas):
