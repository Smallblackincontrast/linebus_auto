...
# @File     :test_giftcard_update_status.py
# @software :PyCharm
import unittest
...
testdata = ReadDatas(DataPaths.DatasPath).read_datas("giftcard_update_status")


@ddt
class TestGiftcardUpdateStatus(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.log = MyLogs()
        ...
        cls.re = DataRe()
        cls.api = ApiRequests()
        gift_card_strategy_data = operation_mysql("select * from bus_cards.sc_gift_card_strategy where tid =  \'"+CD.tid+"\';")
        setattr(CD,"gift_card_strategy_id",gift_card_strategy_data[-1]["id"])
    ...
    @data(*testdata)
    def test_giftcard_update_status(self, datas):
