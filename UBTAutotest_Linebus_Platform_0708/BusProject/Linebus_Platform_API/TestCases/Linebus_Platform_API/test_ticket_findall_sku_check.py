# -*- coding:utf-8 -*-
# @Time     :2019/7/10 17:42
# @Author   :Tester_Wang
# @Email    :wangying@riverroad.com
# @File     :test_ticket_findall_sku_check.py
# @software :PyCharm
import unittest
...
testdata = ReadDatas(DataPaths.DatasPath).read_datas("ticket_findall_sku_check")


@ddt
class TestTicketFindallSkuCheck(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.log = MyLogs()
        ...
        cls.re = DataRe()
        cls.api = ApiRequests()
        ticket_data = operation_mysql("select * from bus_linebus_engine.lb_ticket where tid = \'" + CD.tid + "\';")
        setattr(CD, "skuId", ticket_data[-1]["sku_id"])
    ...
    @data(*testdata)
    def test_ticket_findall_sku_check(self, datas):
