# -*- coding:utf-8 -*-
# @Time     :2019/7/19 15:56
# @Author   :Tester_Wang
# @Email    :wangying@riverroad.com
# @File     :test_ticketrefund_order_detail.py
# @software :PyCharm
import unittest
...
testdata = ReadDatas(DataPaths.DatasPath).read_datas("ticketrefund_order_detail")


@ddt
class TestTicketrefundOrderDetail(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.log = MyLogs()
        ...
        cls.re = DataRe()
        cls.api = ApiRequests()
        ticket_refund_data = operation_mysql("SELECT * FROM lb_ticket_refund WHERE tid = \'" + CD.tid + "\';")
    ...
    @data(*testdata)
    def test_ticketrefund_order_detail(self, datas):
