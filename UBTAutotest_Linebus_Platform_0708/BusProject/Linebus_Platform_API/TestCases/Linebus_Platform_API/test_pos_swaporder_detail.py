# -*- coding:utf-8 -*-
# @Time     :2019/7/24 11:03
# @Author   :Tester_Wang
# @Email    :wangying@riverroad.com
# @File     :test_pos_swaporder_detail.py
# @software :PyCharm
...
testdata = ReadDatas(DataPaths.DatasPath).read_datas("pos_swaporder_detail")


@ddt
class TestPosSwaporderDetail(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.log = MyLogs()
        ...
        cls.re = DataRe()
        cls.api = ApiRequests()
        pos_order_data = operation_mysql(
            "select * from bus_salecenter.sc_order where device_id is not null and tenant_id = \'" + CD.tid + "\';")
        setattr(CD,"pos_order_id",pos_order_data[-1]["id"])
    ...
    @data(*testdata)
    def test_pos_swaporder_detail(self, datas):
