# -*- coding:utf-8 -*-
# @Time     :2019/7/16 15:28
# @Author   :Tester_Wang
# @Email    :wangying@riverroad.com
# @File     :test_ticket_reschedule_refticket.py
# @software :PyCharm
import unittest
...
testdata = ReadDatas(DataPaths.DatasPath).read_datas("ticket_reschedule_refticket")


@ddt
class TestTicketRescheduleRefticket(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.log = MyLogs()
        ...
        cls.re = DataRe()
        cls.api = ApiRequests()
        reschedule_ticket_data = operation_mysql(
            "select * from bus_linebus_engine.lb_ticket where status = 4 and tid = \'" + CD.tid + "\';")
        setattr(CD, "reschedule_ticket_id", reschedule_ticket_data[-1]["id"])
        setattr(CD, "order_id", reschedule_ticket_data[-1]["order_id"])
    ...
    @data(*testdata)
    def test_ticket_reschedule_check_admin(self, datas):
