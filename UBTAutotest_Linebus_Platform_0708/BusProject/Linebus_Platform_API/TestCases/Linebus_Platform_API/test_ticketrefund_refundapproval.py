# -*- coding:utf-8 -*-
# @Time     :2019/7/22 15:38
# @Author   :Tester_Wang
# @Email    :wangying@riverroad.com
# @File     :test_ticketrefund_refundapproval.py
# @software :PyCharm
import unittest
...
testdata = ReadDatas(DataPaths.DatasPath).read_datas("ticketrefund_refundapproval")


@ddt
class TestTicketrefundRefundapproval(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.log = MyLogs()
        ...
        cls.re = DataRe()
        cls.api = ApiRequests()
        ticket_refund_data = operation_mysql(
            "SELECT * FROM lb_ticket_refund WHERE deleted = 0 and state = 0 and tid = \'" + CD.tid + "\';")
        setattr(CD, "ticket_refund_id", ticket_refund_data[-1]["id"])
        setattr(CD, "order_id", ticket_refund_data[-1]["order_id"])
        cls.operator_login = cls.api.api_request("post", CD.url + r'/admin/login', None,
                                                 {"username": "qaadmin", "password": "123456"})

        cls.permission_load = cls.api.api_request("post", CD.url + r'/admin/permission/load',
                                                  {"tid": CD.tid, "operatorId": CD.operatorid}, None)
        setattr(CD, "operatortoken", cls.operator_login.json()["more"]["operatorToken"])
    ...
    @data(*testdata)
    def test_ticketrefund_refundapproval(self, datas):
