...
# @File     :test_ticketop_log_delete.py
# @software :PyCharm
import unittest
...
testdata = ReadDatas(DataPaths.DatasPath).read_datas("ticketop_log_delete")


@ddt
class TestTicketopLogDelete(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.log = MyLogs()
        ...
        cls.re = DataRe()
        cls.api = ApiRequests()
        ticket_operation_log_data = operation_mysql(
            "select * from bus_linebus_platform.lb_ticket_operation_log where deleted = 0 and  tid = \'" + CD.tid + "\';")
        setattr(CD, "ticket_operation_log_id", ticket_operation_log_data[0]["id"])
        # 管理员登录
    ...
    @data(*testdata)
    def test_ticketop_log_delete(self, datas):
